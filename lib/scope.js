...
function defineScope(cls, targetClass, name, params, methods) {
  ...
  var model = targetClass.definition.name;

  fn.shared = true;
  fn.http = {verb: 'get', path: '/' + name};
  fn.accepts = {arg: 'where', type: 'object'};
  fn.description = 'Fetches ' + name;
  fn.returns = {arg: name, type: 'array', items: { type: model }, root: true};
  ...
  fn_create.accepts = {arg: 'data', type: model, http: {source: 'body'}};
  fn_create.description = 'Creates ' + name;
  fn_create.returns = {arg: 'data', type: model, root: true};
  ...
  fn_delete.returns = {arg: 'data', type: model, root: true};
